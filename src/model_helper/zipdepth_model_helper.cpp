...
ZipDepthModelHelper::ZipDepthModelHelper(char *model_file, char *labels_file,
...
bool ZipDepthModelHelper::preprocess(camera_image_metadata_t &meta, char *frame,
...
bool ZipDepthModelHelper::run_inference(cv::Mat &preprocessed_image, double *last_inference_time)
{
    start_time = rc_nanos_monotonic_time();
    ...
    {
        fprintf(stderr, "ZipDepthModelHelper: expected float32 input, got type %d\n",
                input_tensor->type);
        fprintf(stderr, "Export the float32 tflite (onnx2tf ... _float32.tflite).\n");
        return false;
    }
    ...
    {
        fprintf(stderr, "ZipDepthModelHelper: Invoke() failed\n");
        return false;
    }
...
bool ZipDepthModelHelper::postprocess(cv::Mat &output_image, double last_inference_time,
                                      void *input_params)
{
    if (input_params == nullptr)
        return false;

    ZipDepthFrameParams *params = static_cast<ZipDepthFrameParams *>(input_params);
    start_time = rc_nanos_monotonic_time();
    ...
    {
        fprintf(stderr, "ZipDepthModelHelper: expected float32 output, got type %d\n",
                output_tensor->type);
        return false;
    }
...
bool ZipDepthModelHelper::worker(cv::Mat &output_image, double last_inference_time,
                                 camera_image_metadata_t metadata, void *input_params)
{
    ...
    ZipDepthFrameParams frame_params(metadata);
